...
class Client extends EventEmitter {
    constructor(options = {}) {
        super();
        console.log('Bot: Client constructor called with options:', options);
     ...
     */
    async inject() {
        console.log('Bot: Injecting scripts into WhatsApp Web page.');
        await this.pupPage.waitForFunction('window.Debug?.VERSION != undefined', {timeout: this.options.authTimeoutMs});
        await this.setDeviceName(this.options.deviceName, this.options.browserName);
        const pairWithPhoneNumber = this.options.pairWithPhoneNumber;
        const version = await this.getWWebVersion();
        ...
        if (needAuthentication) {
            console.log('Bot: Authentication needed. Waiting for QR code or pairing code.');
            const { failed, failureEventPayload, restart } = await this.authStrategy.onAuthenticationNeeded();
        ...
        await exposeFunctionIfAbsent(this.pupPage, 'onAppStateHasSyncedEvent', async () => {
            console.log('Bot: App state has synced. Authentication successful.');
            const authEventPayload = await this.authStrategy.getAuthEventPayload();
            /**
                 * Emitted when authentication is successful
                 * @event Client#authenticated
     ...
     */
    async initialize() {
        console.log('Bot: Initializing the client...');
        ...
        const puppeteerOpts = this.options.puppeteer;
        if (puppeteerOpts && (puppeteerOpts.browserWSEndpoint || puppeteerOpts.browserURL)) {
            console.log('Bot: Connecting to existing Puppeteer browser.');
            browser = await puppeteer.connect(puppeteerOpts);
            page = await browser.newPage();
        } else {
            console.log('Bot: Launching new Puppeteer browser.');
            const browserArgs = [...(puppeteerOpts.args || [])];
            if(!browserArgs.find(arg => arg.includes('--user-agent'))) {
                browserArgs.push(`--user-agent=${this.options.userAgent}`);
            }
        ...
        await this.authStrategy.afterBrowserInitialized();
        await this.initWebVersionCache();
        console.log('Bot: Navigating to WhatsApp Web URL.');
        // ocVersion (isOfficialClient patch)
        // remove after 2.3000.x hard release
        await page.evaluateOnNewDocument(() => {
            const originalError = Error;
     ...
     */
    async attachEventListeners() {
        console.log('Bot: Attaching event listeners...');
        await exposeFunctionIfAbsent(this.pupPage, 'onAddMessageEvent', msg => {
            if (msg.type === 'gp2') {
                const notification = new GroupNotification(this, msg);
                if (['add', 'invite', 'linked_group_join'].includes(msg.subtype)) {
    ...
    async sendMessage(chatId, content, options = {}) {
        ...
        if (internalOptions.sendMediaAsSticker && internalOptions.media) {
            internalOptions.media = await Util.formatToWebpSticker(
                internalOptions.media,
                { name: options.stickerName, author: options.stickerAuthor, categories: options.stickerCategories },
                this.pupPage
            );
        }
    ...
    /**
     * Gets contact instance by ID
     * @param {string} contactId
     * @returns {Promise<Contact>}
     */
    async getContactById(contactId) {
        let contact = await this.pupPage.evaluate(async contactId => {
            return await window.WWebJS.getContact(contactId);
        }, contactId);
     ...
     */
    async getContacts() {
        const contacts = await this.pupPage.evaluate(async () => {
            return await window.WWebJS.getContacts();
        });

        return contacts.map(contact => new Contact(this, contact));
    }

    /**
     * Returns the contact ID for the current user
     * @returns {Promise<string>}
     */
    async getWwebMeId() {
        const meId = await this.pupPage.evaluate(() => {
            return window.Store.Me.wid._serialized;
        });
    ...
    async getChatById(chatId) {
        ...
        if (chat) {
            chat = ChatFactory.create(this, chat);
        }
    ...
    /**
     * Get all contacts from the user's addressbook
     * @returns {Promise<Array<Contact>>}
     */
    async getAddressbookContacts() {
        const contacts = await this.pupPage.evaluate(async () => {
            return await window.WWebJS.getAddressbookContacts();
        });
    ...
    async acceptGroupInvite(inviteCode) {
        const res = await this.pupPage.evaluate(async inviteCode => {
            return await window.Store.Invite.acceptGroupInvite(inviteCode);
        }, inviteCode);

        return res.wid._serialized;
    }

    /**
     * Accepts a group membership request
     * @param {string} chatId Group id
     * @param {string} requesterId User id that made the request
     * @returns {Promise<object>}
     */
    async acceptGroupMembershipRequest(chatId, requesterId) {
        return await this.pupPage.evaluate(async (chatId, requesterId) => {
            return await window.Store.GroupUtils.acceptJoinRequest(chatId, requesterId);
        }, chatId, requesterId);
    }

    /**
     * Rejects a group membership request
     * @param {string} chatId Group id
     * @param {string} requesterId User id that made the request
     * @returns {Promise<object>}
     */
    async rejectGroupMembershipRequest(chatId, requesterId) {
        return await this.pupPage.evaluate(async (chatId, requesterId) => {
            return await window.Store.GroupUtils.rejectJoinRequest(chatId, requesterId);
        }, chatId, requesterId);
    }
    ...
    async getGroupInvites() {
        return await this.pupPage.evaluate(async () => {
            return await window.Store.Invite.queryGroupInvites();
        });
    }

    /**
     * Gets a Contact by ID, or by phone number, or by a Contact object.
     * @param {string|Contact} contactId
     * @returns {Promise<Contact>}
     */
    async getContact(contactId) {
        if (contactId instanceof Contact) {
            contactId = contactId.id._serialized;
        }
    ...
    /**
     * Gets a Chat by its ID or by a Contact object
     * @param {string|Contact} chatId
     * @returns {Promise<Chat>}
     */
    async getChat(chatId) {
        if (chatId instanceof Contact) {
    ...
    async getUnreadMessages() {
        const messages = await this.pupPage.evaluate(async () => {
            ...
            for (let i = 0; i < unreadChats.length; i++) {
                ...
                const msgs = await chat.loadMessages();
                messages.push(...msgs);
            }
            return messages;
        });
     ...
     * @returns {Promise<Chat>}
     */
    async getChatFromMessage(message) {
        const chat = await this.pupPage.evaluate(async message => {
            return await window.WWebJS.getChat(message.from);
        }, message);
    ...
    /**
     * Gets the contact from the message
     * @param {Message} message
     * @returns {Promise<Contact>}
     */
    async getContactFromMessage(message) {
        const contact = await this.pupPage.evaluate(async message => {
            return await window.WWebJS.getContact(message.from);
        }, message);
    ...
    /**
     * Gets the group from the group notification
     * @param {GroupNotification} groupNotification
     * @returns {Promise<Chat>}
     */
    async getChatFromGroupNotification(groupNotification) {
        const chat = await this.pupPage.evaluate(async groupNotification => {
            return await window.WWebJS.getChat(groupNotification.chatId);
        }, groupNotification);
    ...
    /**
     * Gets the contact from the group notification
     * @param {GroupNotification} groupNotification
     * @returns {Promise<Contact>}
     */
    async getContactFromGroupNotification(groupNotification) {
        const contact = await this.pupPage.evaluate(async groupNotification => {
            return await window.WWebJS.getContact(groupNotification.author);
        }, groupNotification);
    ...
    async getMessageById(messageId) {
        const msg = await this.pupPage.evaluate(async messageId => {
            return await window.WWebJS.getMessage(messageId);
        }, messageId);

        return msg ? new Message(this, msg) : undefined;
    }

    /**
     * Gets the message by ID or by a Message object
     * @param {string|Message} messageId
     * @returns {Promise<Message>}
     */
    async getMessage(messageId) {
        if (messageId instanceof Message) {
            messageId = messageId.id._serialized;
        }
    ...
    async getMessages(chatId, options = {}) {
        const messages = await this.pupPage.evaluate(async (chatId, options) => {
            const chat = await window.WWebJS.getChat(chatId);
            const messages = await chat.loadMessages(options.limit);
            return messages.map(msg => window.WWebJS.getMessageModel(msg));
        }, chatId, options);
    ...
    /**
     * Get the profile picture URL for a given chat
     * @param {string} chatId
     * @returns {Promise<string>}
     */
    async getProfilePicUrl(chatId) {
        const url = await this.pupPage.evaluate(async chatId => {
            return await window.WWebJS.getProfilePicUrl(chatId);
        }, chatId);
    ...
    async getProfilePic(chatId) {
        const pic = await this.pupPage.evaluate(async chatId => {
            return await window.WWebJS.getProfilePic(chatId);
        }, chatId);
     ...
     */
    async getMe() {
        const me = await this.pupPage.evaluate(async () => {
            return await window.WWebJS.getMe();
        });

        return new Contact(this, me);
    }
     ...
     * @returns {Promise<string>}
     */
    async getMePhoneNumber() {
        const phoneNumber = await this.pupPage.evaluate(async () => {
            const me = await window.WWebJS.getMe();
     ...
     */
    async getMeId() {
        const id = await this.pupPage.evaluate(async () => {
            const me = await window.WWebJS.getMe();
            return me.id._serialized;
        });
     ...
     * @param {boolean} syncToAddressbook Should the contact be synced to the user's addressbook?
     * @returns {Promise<object>}
     */
    async addAddressbookContact(phoneNumber, firstName, lastName, syncToAddressbook) {
        return await this.pupPage.evaluate(async (phoneNumber, firstName, lastName, syncToAddressbook) => {
            return await window.Store.AddressbookContactUtils.addContactAction(
                phoneNumber,
                firstName,
                lastName,
                syncToAddressbook
